...
$$p(\vec x)= P_{\theta_0}(\lambda(\vec X)>\lambda(\vec x))\le \frac{E_{\theta_0}[\lambda(\vec X)]}{\lambda(\vec x)} =  \frac{1}{\lambda(\vec x)},$$
where we used the [Markov's inequality](https://en.wikipedia.org/wiki/Markov%27s_inequality). The equality holds as long as $P_{\theta_0}[\lambda(\vec X)]=\lambda(\vec x)]=1$. So the
p-value is generally less than the inverse of the  likelihood ratio. This implies that if $\alpha>\frac{1}{\lambda(\vec x)}$ (the inverse of the  likelihood ratio), we should reject the null.
