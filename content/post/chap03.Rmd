...
date: "2018-11-05"
categories: ["课件"]
Summary: "第三章：假设检验(Hypothesis Tests)"
tags: ["课件", "数理统计"]
---
...
- 当$\theta\in \Theta_1$时，$1-\rho_W(\theta)$表示犯第二类错误的概率
...
等价于找$C$, 使得$P_{\mu_1}(\bar X> C)=\alpha$, $C=\mu_1+u_{1-\alpha}\sigma/\sqrt{n}$, 所以似然比检验的拒绝域为$W=\{\vec x:\vec x> \mu_1+u_{1-\alpha}\sigma/\sqrt{n}\}$

与双侧拒绝域比较：$W'=\{\vec x:|\vec x-\mu_1|> u_{1-\alpha/2}\sigma/\sqrt{n}\}$, **哪个好？**
...
等价于找$C$, 使得$P_{\mu_1}(\bar X< C)=\alpha$, $C=\mu_1+u_{\alpha}\sigma/\sqrt{n}$, 所以似然比检验的拒绝域为$W=\{\vec x:\vec x< \mu_1+u_{\alpha}\sigma/\sqrt{n}\}$
...
**解**：前面两种情况的UMP拒绝域为$$W=\{\vec x:\vec x> \mu_1+u_{1-\alpha}\sigma/\sqrt{n}\}$$

后面两种情况的UMP拒绝域为$$W=\{\vec x:\vec x< \mu_1+u_{\alpha}\sigma/\sqrt{n}\}$$
...
$$W=\{\vec x:\sum_{i=1}^n(x_i-\mu)^2>C\}=\{\vec x:\frac{1}{n}\sum_{i=1}^n\frac{(x_i-\mu)^2}{\sigma^2_1}>C'\},$$ 

其中$C'$满足$P_{\sigma^2_1}(\frac{1}{n}\sum_{i=1}^n\frac{(x_i-\mu)^2}{\sigma^2_1}>C')=\alpha$, 所以$C'=\chi^2_{1-\alpha}(n)$
...
L(\vec x;\bar x,q_n^2),\ &\bar x> \mu_0
\end{cases}$$
...
其中$\chi^2 = \frac{1}{\sigma^2_0}\sum_{i=1}^n(x_i-\bar x)^2=ns_n^2/\sigma_0^2$, $C_1,C_2$满足
...
$$U=\frac{(\bar X-\bar Y)-(\mu_1-\mu_2)}{\sqrt{\sigma_1^2/m+\sigma_2^2/n}}=\frac{\bar X-\bar Y-\delta}{\sqrt{\sigma_1^2/m+\sigma_2^2/n}}$$
...
$$F=\frac{\frac 1 m\sum_{i=1}^m(X_i-\mu_1)^2}{\frac 1 n\sum_{i=1}^n(Y_i-\mu_2)^2}\frac{\sigma_2^2}{\sigma_1^2}=\frac{\frac 1 m\sum_{i=1}^m(X_i-\mu_1)^2}{\frac 1 n\sum_{i=1}^n(Y_i-\mu_2)^2}\sim F(m,n)$$
	
若$F> F_{1-\alpha/2}(m,n)$或者$F<F_{\alpha/2}(m,n)$拒绝$H_0$，否则接受$H_0$.
	
- 如果$\mu_1,\mu_2$未知，选择**F检验统计量**：
$$F=\frac{\frac 1 {m-1}\sum_{i=1}^m(X_i-\bar X)^2}{\frac 1 {n-1}\sum_{i=1}^n(Y_i-\bar Y)^2}=\frac{S_{1m}^{*2}}{S_{2n}^{*2}}\sim F(m-1,n-1)$$
...
$$W=\{\vec x:|\vec x-\mu_0|>u_{1-\alpha/2}\frac{\sigma}{\sqrt{n}}\}$$
...
- $\lambda$满足$\sup_{\theta\in\Theta_0}P_{\theta}(T(\vec X)>\lambda)=\alpha$
...
如果给定$\alpha$, $\lambda$存在且唯一，则可以证明：$T(\vec x)>\lambda$当且仅当$p(\vec x)<\alpha$
